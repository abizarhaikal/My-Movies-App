...
import com.example.mymoviesapp.adapter.GenresSeriesAdapter
...
class DetailActivity : AppCompatActivity() {
    ...
    private fun setRecyclerViewGenres() {
        ...
        val linearLayoutManager = LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        recyclerViewGenres.layoutManager = linearLayoutManager
            ...
            if (result != null) {
                when (result) {
                    ...
                    is ResultState.Success -> {
                        ...
                        genresAdapter.submitList(result.data.genres.subList(0, 2))
                        Glide.with(this)
                            .load(fullImageUrl)
                            .into(binding.imageBanner)
                        binding.tvTitle.text = result.data.title
                        val rate = result.data.popularity
                        val releaseYear = result.data.releaseDate
                        val runtime = result.data.runtime
                        ...
                        binding.tvDuration.text = formatRuntime(runtime)
                ...
                override fun onResourceReady(resource: Bitmap, transition: Transition<in Bitmap>?) {
                    ...
                    Palette.from(resource).generate { pallete ->
                        pallete?.dominantSwatch?.let { swatch ->
                            ...
                            this@DetailActivity.window.statusBarColor = color
                        }
                    }
                }
